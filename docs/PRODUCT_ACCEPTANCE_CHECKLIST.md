...
| Real-time idle/running/waiting/completed status | Missing at product level | Current PTY heuristics can show startup, activity, attention text, and process exit. CX-01A fixture-verifies a private Codex success/command-and-file-approval decoder through the normalized gate, but it is not wired to a process or product runtime; Codex failure mapping, Claude hooks, and Pi RPC events remain absent, so normal completed turns can remain `Working`. |
...
| Authoritative live lifecycle | Fixture foundation only (CX-01A success/approval subset; no production binding or failure mapping) | Missing | Missing |
