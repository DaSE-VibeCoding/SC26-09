...
LC-02A defines the version-1 frontend wire contracts, fail-closed decoders, transport capability helper, and pure aggregate session/lifecycle/stream reducer. LC-02B now provides the in-process Rust SessionHost migration bridge: one binding map owns PTYs, stream-scoped controls and canonical ordered events, while legacy commands project compatibility events only for legacy-opened streams. React consumes only the canonical event family. This slice supports PTY fallback only; protocol transports, replay, and real structured provider adapters remain future work.
