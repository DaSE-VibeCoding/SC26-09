...
LC-02A defines the first frontend wire contracts, fail-closed decoders, transport capability helper, and pure aggregate session/lifecycle/stream reducer. LC-02B provides the in-process Rust SessionHost migration bridge: one binding map owns PTYs, stream-scoped controls, and canonical ordered events, while legacy commands project compatibility events only for legacy-opened streams. LC-02C bumps the in-process protocol to v2 and adds the provider-neutral structured lifecycle identity gate at the codec, runtime, and Rust SessionHost seams: structured PTY/protocol bindings carry bounded source identity, host activity carries matching source plus turn context, and fallback PTY bindings remain source-less. CAP-01A adds a frontend/domain prompt-readiness fixture foundation without changing SessionHost protocol v2, Rust, or the codec: fallback PTY streams become `pty-fallback-sendable`, structured streams start `awaiting-authoritative`, and policy fixtures define `ready`, `auth-required`, `setup-required`, and `unsupported`. CX-01A adds an always-compiled private Rust child module that fixture-decodes the narrow public Codex app-server v2 handshake plus successful turn/completion and command/file approval lifecycle directly into the LC-02C gate. It has no Tauri registration or production binding; failed/interrupted turns remain typed decoder outcomes rather than product lifecycle events. No production runtime producer can set authoritative readiness states yet. React consumes only the canonical event family. Current production sessions still use PTY fallback only; protocol transports, replay, readiness producers, failure mapping, and real structured provider adapters remain future work.
