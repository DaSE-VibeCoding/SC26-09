...
CX-01B fixture-verifies an always-compiled but unregistered supervisor that directly owns one configured app-server child, bounds stdout JSONL, continuously discards stderr, correlates the canonical new-thread handshake, commits exact source plus `ready` atomically, and reaps gracefully or forcibly. A sanitized native supervisor smoke passed against Codex 0.144.5 without sending a prompt or recording provider data. The current private fixture combines one route with one process; production work must first introduce process-global request routing for the target one-process-per-`CODEX_HOME` topology and gate terminal-less UI before registration.
