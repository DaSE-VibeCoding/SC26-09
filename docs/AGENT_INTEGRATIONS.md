...
The LC-02A frontend slice defines semantic requests, snapshots/events, bounded runtime validation, stream ordering, and the optional-terminal capability. LC-02B adds the in-process native SessionHost, one-active-stream enforcement, stream-scoped controls, and an atomic React cutover to canonical events. LC-02C bumps the in-process contract to protocol v2 and fixture-verifies the provider-neutral structured identity gate before native adapters: structured bindings carry normalized source identity (`agentId`, integration, exact provider session, provenance), structured activity carries matching source plus turn context, and fallback PTY bindings remain source-less. CAP-01A fixture-verifies the frontend/domain capability and prompt-readiness contract: every production adapter advertises its preferred future transport, exactly `supportedBindings: ["pty-fallback"]`, and `resumable`; accepted fallback bindings are immediately sendable through the terminal but not provider-ready; structured PTY/protocol bindings start blocked as `awaiting-authoritative`; `ready`, `auth-required`, `setup-required`, and `unsupported` are pure policy fixture states only. CX-01A fixture-verifies a private, narrow Codex app-server v2 decoder for exact handshake/thread identity, successful turns, and command/file approval correlation through the existing Rust gate. It is not registered in Tauri and does not spawn or bind app-server, send prompts, alter readiness or supported bindings, or claim native verification; failed/interrupted turns are not yet mapped into product lifecycle. No client action, text/output/process signal, Rust host path, or protocol-v2 event can manufacture authoritative readiness yet. Current sessions use that host's PTY-fallback binding; preferred protocol transports and readiness producers are not live.
