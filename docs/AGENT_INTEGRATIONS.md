...
The LC-02A frontend slice defines semantic requests, snapshots/events, bounded runtime validation, stream ordering, and the optional-terminal capability. LC-02B adds the in-process native SessionHost, one-active-stream enforcement, stream-scoped controls, and an atomic React cutover to canonical events. LC-02C introduced the provider-neutral structured identity gate in protocol v2: structured bindings carry normalized source identity (`agentId`, integration, exact provider session, provenance), structured activity carries matching source plus turn context, and fallback PTY bindings remain source-less. CAP-01A fixture-verifies the frontend/domain capability and prompt-readiness contract: every production adapter advertises its preferred future transport, exactly `supportedBindings: ["pty-fallback"]`, and `resumable`; accepted fallback bindings are immediately sendable through the terminal but not provider-ready; structured PTY/protocol bindings start blocked as `awaiting-authoritative`; `ready`, `auth-required`, `setup-required`, and `unsupported` are pure policy fixture states only. CX-01A fixture-verifies a private, narrow Codex app-server v2 decoder for exact handshake/thread identity, turns, and command/file approval correlation through the Rust gate. LC-02D advances the SessionHost contract to protocol v3 and maps exact completed, failed, and interrupted outcomes into the normalized lifecycle while preserving sticky attention and rejection atomicity. Neither slice is registered as a production binding: Pelican does not spawn or bind app-server for live sessions, send protocol prompts, alter readiness or supported bindings, or claim native verification. No client action, text/output/process signal, Rust host path, or protocol-v3 event can manufacture authoritative readiness yet. Current sessions use the host's PTY-fallback binding; preferred protocol transports and readiness producers are not live.
