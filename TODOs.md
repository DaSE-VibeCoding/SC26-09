...
| **OA-03** | P4 | **Ready** | DI-10 | Right inspector controls. OA-03A provides the collapsible read-only tree in [`src/components/WorkspaceFileTree.tsx`](src/components/WorkspaceFileTree.tsx). | Expand/collapse, folder icons, flattened-tree ARIA metadata, and keyboard transitions are fixture verified. Aggregate OA-03 remains incomplete until open/reveal/copy path, file search, and optional rich hunks/binary state are explicit and bounded. | Evidence: `6e154f0`; 157 frontend and 44 Rust tests passed. Browser focus/assistive-technology smoke and remaining file actions are not verified. Stop if file operations become write-capable without explicit user action. |
