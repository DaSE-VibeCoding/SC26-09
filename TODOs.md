...
| **CAP-01** | **Ready** | LC-02C | Provider capability and readiness contract in the agent registry and launch/resume flows. Carries the original agent-ready handshake backlog item. | Each provider advertises structured lifecycle support/fallback mode; prompt submission waits for an authoritative ready/auth/setup state instead of startup-output inference; unsupported capability gets explicit fallback copy. | Add capability fixtures and startup-state tests after LC-02C. Native provider smoke is required before **Provider verified**. Stop if readiness is only inferable from TUI text. |
