...
| **PI-01** | **Ready** | LC-02C, CAP-01 | Pi structured lifecycle adapter via supported RPC/session JSONL surfaces; coordinate resume-handle timing with DI-04. | Pi turn start, completion, error, and attention events map into the LC-02C gate with exact JSONL/session identity and no inferred replacement session. | RPC/session fixtures first; native Pi smoke later. Stop if the resume handle is missing or synthesized. |
