...
| `d10bd83` — LC-02A frontend host contract foundation | **LC-02A** fixture-verified frontend wire contract, fail-closed codec/runtime seams, transport capability helper, and aggregate reducer foundation exist. | No native SessionHost migration bridge, structured identity gate, provider runtime wiring, native smoke, or product acceptance. |
...
| `ac4f64e` — LC-02C structured identity gate | Protocol-v2 source, provider-session, integration, turn, attention, and ordering gates are fixture verified without consuming sequence or cursor on rejection. | No production structured provider producer, native provider smoke, or product acceptance. |
| `87f3f58` — CAP-01A capability/readiness foundation | Registry capabilities, ephemeral prompt-readiness state, pure prompt policy/input construction, and accessible composer readiness copy are fixture verified. All production bindings remain `pty-fallback`. | No real readiness producer, provider handshake, native smoke, protocol/Rust change, or product acceptance. |
| `356b99b` — CX-01A Codex fixture decoder | A private Codex app-server v2 decoder fixture-verifies exact handshake, thread/turn identity, successful completion, and command/file approval correlation through the structured gate. | No production binding, prompt/readiness producer, process ownership, native provider smoke, or product acceptance. |
| `5eabf7c` — LC-02D normalized terminal outcomes | Protocol-v3 `completed`, `failed`, and `interrupted` outcomes, ordering, status mapping, and sticky-attention precedence are fixture verified from Codex decoding through React reduction. | No production structured provider producer, native provider smoke, or product acceptance. |
| `e7dd6a1` — PI-01A Pi RPC fixture decoder | A private Pi decoder fixture-verifies exact persisted session identity, serialized prompt epochs, settlement outcomes, retry/tool-use handling, and extension UI attention through the shared gate. | No Pi RPC process ownership, production prompt path, readiness evidence, native provider smoke, or product acceptance. |
| `6e154f0` — OA-03A collapsible file tree | The right inspector has a fixture-verified, read-only collapsible tree with represented-child semantics, workspace-scoped state, folder icons, ARIA metadata, and keyboard transitions. | No file open/reveal/copy path, search, rich diff, browser assistive-technology smoke, or product acceptance. |
| `36385ee` — native Codex wire alignment | A sanitized native Codex 0.144.5 app-server turn verified typed IDs, `thread.id` identity, omitted `jsonrpc`, nested `turn.status`, terminal completion, and clean EOF exit; the private decoder now matches that observed shape. | Pelican still has no production app-server binding, native in-app workflow, readiness evidence, provider verification, or product acceptance. |
| `06d3145` — CAP-01B host readiness channel | Protocol-v4 snapshots and ordered events fixture-verify normalized host-owned readiness, exact source/stream gates, atomic rejection, rejoin restoration, and honest PTY fallback. | No provider readiness producer, process ownership, live protocol binding, native smoke, or product acceptance. |
| `188e60a` — CX-01B Codex supervisor | Fake-child fixtures and a sanitized native Codex 0.144.5 handshake prove bounded stdio ownership, exact `thread.id` binding, one atomic host-readiness transition, normalized ordering, and graceful/forced reap. | The supervisor is private and unregistered: no in-app binding, prompt transport, pooling, resume, capability fallback, provider verification, or product acceptance. |
| `aa13275` — CX-01C terminal-less surface gate | Pure transport-policy fixtures and App wiring prove protocol bindings resolve synchronously to Prompt, omit terminal controls and buffers, and use transport-correct readiness copy while PTY behavior remains unchanged. | No production protocol binding, browser/native PTY regression smoke, provider verification, or product acceptance. |
...
| Real-time idle / working / waiting / completed while CLI stays open | Missing as product behavior — LC-00/LC-01 and LC-02A/B/C/D are fixture-verified foundations only |
...
| **LC-02C** | **Fixture verified** | LC-00, LC-01; LC-02A/LC-02B evidence above | Provider-neutral structured lifecycle identity/provenance gate across [`src/domain/sessionHost.ts`](src/domain/sessionHost.ts), [`src/services/sessionHostCodec.ts`](src/services/sessionHostCodec.ts), [`src/domain/sessionRuntime.ts`](src/domain/sessionRuntime.ts), [`src-tauri/src/session_host.rs`](src-tauri/src/session_host.rs), and protocol-version call sites. | LC-02A/B/C foundations exist: the identity gate introduced in protocol v2 remains authoritative in current protocol v4; structured PTY/protocol bindings carry bounded source identity; host activity carries matching source plus turn context; fallback PTY remains source-less/local; stale/untrusted events are ignored fail-closed by provider/session/integration/turn identity at pure seams. No production structured provider adapter is included. | Evidence: `ac4f64e`; outcome extension: `5eabf7c`; current v4 readiness extension: `06d3145`. TypeScript codec/runtime tests and Rust SessionHost gate tests cover valid Codex/Claude/Pi modes and rejection without sequence/cursor advancement. Native in-app smoke is still required through CX-02, CL-02, and PI-02. Stop if provider event identity/provenance is ambiguous. |
| **LC-02D** | **Fixture verified** | LC-02C; CX-01A evidence above | Provider-neutral structured terminal outcomes across [`src/domain/lifecycle.ts`](src/domain/lifecycle.ts), [`src/domain/sessionRuntime.ts`](src/domain/sessionRuntime.ts), [`src/services/sessionHostCodec.ts`](src/services/sessionHostCodec.ts), [`src-tauri/src/session_host.rs`](src-tauri/src/session_host.rs), and the private Codex decoder. | Protocol v3 normalizes exact `completed`, `failed`, and `interrupted` outcomes to `Done`, actionable attention, and `Idle`; duplicate/conflicting/stale outcomes fail before sequence/cursor mutation; correlated and fallback attention remain sticky until explicit resolution; raw provider errors stay out of React. | Evidence: `5eabf7c`. 150 frontend and 34 Rust tests passed with strict type checking, production build, formatting, and Clippy. Fixture-only; no production binding or native claim. Stop if an outcome lacks exact source/session/turn identity. |
| **CAP-01A** | **Fixture verified** | LC-02C | Frontend/domain capability and prompt-readiness contract only in [`src/agents/`](src/agents/), [`src/domain/sessionRuntime.ts`](src/domain/sessionRuntime.ts), [`src/domain/sessionPrompt.ts`](src/domain/sessionPrompt.ts), [`src/App.tsx`](src/App.tsx), and [`src/components/PromptComposer.tsx`](src/components/PromptComposer.tsx). | Adapters expose `preferredTransport`, exactly `supportedBindings: ["pty-fallback"]`, and `resumable`; accepted fallback opens/snapshots become `pty-fallback-sendable`; structured bindings start `awaiting-authoritative`; replacement streams reset readiness; snapshots restore host-owned readiness; closed/missing/mismatched readiness fails closed. Pure prompt construction preserves current PTY bytes and emits semantic protocol prompts for ready fixtures. Fallback is sendable but never provider-ready. | Evidence: `87f3f58`; host authority extension: `06d3145`. No frontend action or production provider producer can set authoritative `ready`/`auth-required`/`setup-required`/`unsupported`; all production opens still use PTY fallback. Stop if preserving PTY usability would require pretending it is authoritatively ready. |
| **CAP-01B** | **Fixture verified** | LC-02C, CAP-01A | SessionHost-authoritative prompt readiness channel across the Rust host, protocol codec, and frontend runtime reducer. | Protocol-v4 snapshots and ordered events carry only normalized, stream-scoped readiness; exact structured source identity is required; rejoin restores host-owned state; fallback remains sendable but never provider-ready. No provider process or live protocol binding is included. | Evidence: `06d3145`. 166 frontend and 46 Rust tests passed with strict TypeScript, production build, rustfmt, and strict Clippy. No native smoke applies because no live provider path exists. Stop if readiness would require a public setter, raw provider payload, output/liveness inference, fabricated identity, or production provider registration. |
| **CAP-01** | **Roll-up** | CAP-01B plus provider readiness producers | Provider capability and readiness contract in the agent registry and launch/resume flows. Carries the original agent-ready handshake backlog item. | Complete only when the host readiness channel, provider-specific authoritative readiness producers, and capability/version fallback evidence exist without startup-output inference. | Native provider smoke is required before **Provider verified**. Stop if readiness is only inferable from TUI text, process liveness, terminal silence, or write success. |
| **CX-01B** | **Fixture verified** | LC-02D, CAP-01B; CX-01A evidence | Private Codex app-server process ownership and handshake foundation in [`src-tauri/src/session_host/codex_app_server.rs`](src-tauri/src/session_host/codex_app_server.rs), with a minimal host-private atomic source/readiness helper. | The always-compiled but unregistered supervisor directly owns a configured child, bounded JSONL and discarded stderr, performs canonical `initialize → initialized → thread/start` for one new thread, binds only matching `result.thread.id`, and produces normalized readiness/lifecycle envelopes through private gates. Fake-child fixtures prove correlation, atomicity, ordering, bounded failures, write-state ordering, and graceful EOF/forced/drop reap. | Evidence: `188e60a`. 166 frontend and 53 non-ignored Rust tests passed; the explicit ignored native supervisor smoke passed against Codex 0.144.5 in a temporary workspace. This is transport compatibility only, never in-app **Provider verified** evidence. |
| **CX-01C** | **Fixture verified** | CX-01B, CAP-01B | Provider-neutral terminal-less session-surface gate in [`src/domain/sessionRuntime.ts`](src/domain/sessionRuntime.ts), [`src/domain/sessionPrompt.ts`](src/domain/sessionPrompt.ts), and [`src/App.tsx`](src/App.tsx). | Accepted PTY bindings retain Prompt and Terminal over one stream. Accepted protocol bindings synchronously resolve to Prompt, never mount or initialize `TerminalView`, expose no Terminal mode action/hint/toggle, ignore the Terminal shortcut, and receive transport-correct readiness recovery copy. Missing live connection state fails closed; disconnected PTY recovery remains unchanged. | Evidence: `aa13275`. 175 frontend and 53 non-ignored Rust tests passed with production build, rustfmt, and strict Clippy. Native PTY UI regression smoke remains unverified; no production protocol registration or provider claim exists. |
| **CX-01** | **Roll-up** | CX-01C plus production binding prerequisites | Codex structured lifecycle adapter using supported app-server/public interfaces; coordinate identity work with DI-03. | Complete only when private decoding/supervision and terminal-less UI gating are followed by process-global multiplex routing, a production Codex binding, semantic prompt transport, version/schema fallback, and exact provider identity persistence. | Existing evidence: `356b99b`, `5eabf7c`, `36385ee`, `188e60a`, `aa13275`. Native wire/process compatibility is observed, but no in-app provider workflow or **Provider verified** claim exists. Stop if only unsupported/private Codex state is available or identity cannot be bounded exactly. |
| **CL-01** | **Ready** | LC-02C, CAP-01B | Claude Code structured lifecycle adapter via supported hooks / inventory. | Claude Code turn start, completion, error, approval/question, resume, and attach signals map into the LC-02C gate while preserving exact session/background-agent handles. | Hook/inventory fixtures first; native Claude Code smoke later. Stop if a foreground PTY cannot be attached through a supported handle. |
| **PI-01** | **Ready** | LC-02C, LC-02D, CAP-01B | Pi structured lifecycle adapter via supported RPC/session JSONL surfaces; coordinate resume-handle timing with DI-04. PI-01A adds a private fixture decoder in [`src-tauri/src/session_host/pi_rpc.rs`](src-tauri/src/session_host/pi_rpc.rs). | PI-01A fixture-verifies exact `get_state` plus session-header identity, one typed prompt epoch, `agent_settled` terminal outcomes, retry/tool-use candidate replacement, and untimed extension UI request-resolution correlation. A sanitized Pi 0.80.10 smoke confirmed the RPC ordering and fields; it also showed that a new `sessionFile` may materialize only after the first prompt and workspace paths require canonical comparison. Aggregate PI-01 remains incomplete: no process, production prompt/response path, readiness, discovery integration, queued prompts, or RPC binding is included. | Evidence: `e7dd6a1`; 157 frontend and 44 Rust tests passed at the current baseline. Native wire compatibility is observed, but no in-app provider workflow or **Provider verified** claim exists. Stop if the resume handle is synthesized, prompt epochs overlap, or attention can expire without an observable resolution. |
| **LC-08** | **Ready** | LC-02C, CX-01, CL-01, PI-01 | Attention authority in [`src/domain/status.ts`](src/domain/status.ts), [`src/App.tsx`](src/App.tsx), and provider adapters. | English PTY attention regex is removed or demoted to fallback; structured approval/question events set sticky attention; only authoritative resolution clears it. | Add fixture streams for attention/resolve/error after LC-02C. Native attention smoke required per provider. Stop if provider events lack a resolvable attention identifier. |
| **CX-02** | **Ready** | CX-01, LC-08 | Codex lifecycle verification slice for new/resume/attach and edge cases. | Codex golden workflow shows `Idle → Working → Done` while CLI remains open, sticky attention resolves only from authoritative events, and disconnected sessions are never rendered interactive. | Requires native in-app Codex workflow smoke; the direct protocol observation is insufficient. Do not mark **Provider verified** until evidence is recorded. Stop on ambiguous turn IDs. |
...
| **DI-10** | **Ready** | — | File tree and Git diff loading states in inspector services/UI. | Diff loading, empty, native error/retry, binary, and 2 MiB truncation states are now distinct around a lazy local `@pierre/diffs` renderer. Aggregate DI-10 remains incomplete until file-scan depth/entry truncation is visible. | Diff component fixtures exist; add file-scan truncation fixtures and run a native rich-diff visual smoke. Stop if a failure would be rendered as successful empty content. |
...
| **RF-03** | P2 | **Ready** | LC-02C | UI status copy alignment. | User-facing copy uses product vocabulary (`Waiting` ≈ attention, `Running` ≈ working, `Completed` ≈ done) without breaking domain status types or historical records. | Snapshot/component tests for status labels. Stop if copy changes imply unsupported lifecycle guarantees. |
...
| **RT-02** | P5 | **Ready** | LC-02C, CAP-01 | Native / fixture-backed provider lifecycle test harness. | Working, waiting/attention, retry, completion, and failure are testable per transport with fixtures first and optional native smoke evidence second. | Fixture harness plus documented native smoke commands. Stop if provider credentials or network are required for unit tests. |
...
| --- | --- | --- | --- | --- | --- | --- |
| **CORE-01** | P2 | **Oracle gate** | LC-02C, RF-02 | Extract Pelican Core into a background daemon with a versioned local-socket protocol. | Core, not the webview, owns PTYs/processes; Pelican-owned sessions survive UI restart without duplicate PTYs. `79d3384` webview PTY rejoin remains partial evidence only. | Architecture approval, protocol fixtures, and native UI restart smoke. Stop if at-most-one-PTY ownership cannot be proven. |
...
| ID | Priority | Status | Depends on | Ownership / bounded scope | Definition of done | Validation, native smoke, and stop condition |
| --- | --- | --- | --- | --- | --- | --- |
| **PT-01** | P3 | **Ready** | LC-02C, LC-08 | Center Prompt surface transcript. | Conversation transcript, streamed responses, tool activity, approvals, progress, and usage are rendered from provider events; PTY write success is never treated as provider turn acceptance. | Event fixtures and component tests. Native provider smoke before **Provider verified**. Stop if transcript would be reconstructed from terminal text alone. |
| **PT-02** | P3 | **Ready** | LC-02C, CAP-01 | Turn interrupt control. | A turn can be interrupted without killing the whole CLI process, using the provider-supported cancellation path and preserving session resumability. | Provider capability fixtures and native smoke per provider. Stop if only process kill is available. |
| **PT-03** | P3 | **Ready** | CAP-01 | Prompt history, attachments/context selection, command help, and cross-agent context handoff. | A fixture-backed handoff slice now resolves one to three exact stopped Codex/Claude/Pi histories, exports bounded visible user/assistant text as editable Markdown, and places it in a fresh different agent's draft without auto-send. Aggregate PT-03 remains incomplete until ordinary prompt history, attachments/context selection, and command help exist; native three-provider export compatibility also remains unverified. | Rust parser/privacy fixtures plus frontend selection/dialog tests. Native smoke each provider history format before any provider-verification claim. Stop if context would come from terminal buffers, hidden reasoning, tool payloads, inferred handles, or unbounded reads. |
...
| **OA-02** | P4 | **Ready** | DI-09 | Left sidebar organization. | A scrollable active-workspace browser now searches/filters stopped resumable sessions across Codex, Claude Code, and Pi and resumes only exact handles. Aggregate OA-02 remains incomplete until independent sidebar collapse, global attention inbox, and session rename/archive/pin exist. | Selector/component fixtures exist; native exact-resume smoke remains required. Stop if archive/pin can hide connected attention or a resume handle would be inferred. |
| **OA-03** | P4 | **Ready** | DI-10 | Right inspector controls. OA-03A provides the collapsible read-only tree in [`src/components/WorkspaceFileTree.tsx`](src/components/WorkspaceFileTree.tsx). | The tree now starts folded to top-level and uses Amp-compatible Lucide folder/FileCode outlines; rich local hunks and binary/truncation feedback use lazy `@pierre/diffs`. Aggregate OA-03 remains incomplete until open/reveal/copy path, file search, and optional stage/unstage are explicit and bounded. | Existing OA-03A evidence: `6e154f0`; current component fixtures cover default folding/icons and diff states. Native rich-diff and browser assistive-technology smoke plus remaining file actions are not verified. Stop if file operations become write-capable without explicit user action. |
...
| --- | --- | --- |
| P0 | Wire structured lifecycle as authority for Codex, Claude Code, and Pi; keep PTY text fallback only. | LC-02C, CAP-01, CX-01, CX-02, CL-01, CL-02, PI-01, PI-02 |
| P0 | Detect normal turn completion while the interactive CLI remains alive. | LC-01, LC-02C, CX-02, CL-02, PI-02 |
...
| P0 | Emit or remove unused `process-started`; stop relying only on fragile engagement refs for idle→working. | LC-01, LC-02C |
