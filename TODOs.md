...
| **CX-01C** | **Ready** | CX-01B, CAP-01B | Provider-neutral terminal-less session-surface gate in [`src/domain/sessionRuntime.ts`](src/domain/sessionRuntime.ts), [`src/domain/sessionPrompt.ts`](src/domain/sessionPrompt.ts), and [`src/App.tsx`](src/App.tsx). | Accepted PTY bindings retain Prompt and Terminal over one stream. Accepted protocol bindings synchronously resolve to Prompt, never mount or initialize `TerminalView`, expose no Terminal mode action/hint/toggle, ignore the Terminal shortcut, and receive transport-correct readiness recovery copy. Missing live connection state fails closed; disconnected PTY recovery remains unchanged. | Pure selector/readiness fixtures, frontend checks, and a native PTY regression smoke. No production provider registration or verification claim. Stop if capability requires provider identity, adapter preference, output, or liveness inference instead of the accepted transport descriptor. |
| **CX-01** | **Roll-up** | CX-01C plus production binding prerequisites | Codex structured lifecycle adapter using supported app-server/public interfaces; coordinate identity work with DI-03. | Complete only when private decoding/supervision and terminal-less UI gating are followed by process-global multiplex routing, a production Codex binding, semantic prompt transport, version/schema fallback, and exact provider identity persistence. | Existing evidence: `356b99b`, `5eabf7c`, `36385ee`, `188e60a`. Native wire/process compatibility is observed, but no in-app provider workflow or **Provider verified** claim exists. Stop if only unsupported/private Codex state is available or identity cannot be bounded exactly. |
