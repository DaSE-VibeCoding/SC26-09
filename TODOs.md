...
| Real-time idle / working / waiting / completed while CLI stays open | Missing as product behavior — LC-00/LC-01 and LC-02A/B/C are fixture-verified foundations only |
...
| **CX-01** | **Ready** | LC-02C, CAP-01 | Codex structured lifecycle adapter using supported app-server/public interfaces; coordinate identity work with DI-03. CX-01A adds an always-compiled, private fixture decoder in [`src-tauri/src/session_host/codex_app_server.rs`](src-tauri/src/session_host/codex_app_server.rs). | CX-01A fixture-verifies the narrow public app-server v2 handshake, exact thread/turn/request identity, successful turn completion, and command/file approval request-resolution mapping through the LC-02C gate. Aggregate CX-01 remains incomplete: failure outcomes are typed decoder metadata but not mapped into a product lifecycle, and no process, prompt, readiness, discovery, or production protocol binding is included. | Evidence: `356b99b`. Fixture app-server payloads only; no native claim. Native Codex smoke remains later. Stop if only unsupported/private Codex state is available or identity cannot be bounded exactly. |
