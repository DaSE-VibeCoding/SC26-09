...
| **CX-01** | **Ready** | LC-02C, CAP-01 | Codex structured lifecycle adapter using supported app-server/public interfaces; coordinate identity work with DI-03. | Codex turn start, completion, error, and attention events map into the LC-02C gate with exact thread/session identity and without private database reads or duplicate imported rows. | Fixture app-server payloads first; native Codex smoke later. Stop if only unsupported/private Codex state is available. |
