...
| **LC-02C** | **Fixture verified** | LC-00, LC-01; LC-02A/LC-02B evidence above | Provider-neutral structured lifecycle identity/provenance gate across [`src/domain/sessionHost.ts`](src/domain/sessionHost.ts), [`src/services/sessionHostCodec.ts`](src/services/sessionHostCodec.ts), [`src/domain/sessionRuntime.ts`](src/domain/sessionRuntime.ts), [`src-tauri/src/session_host.rs`](src-tauri/src/session_host.rs), and protocol-version call sites. | LC-02A/B/C foundations exist: the identity gate introduced in protocol v2 remains authoritative in current protocol v3; structured PTY/protocol bindings carry bounded source identity; host activity carries matching source plus turn context; fallback PTY remains source-less/local; stale/untrusted events are ignored fail-closed by provider/session/integration/turn identity at pure seams. No production structured provider adapter is included. | Evidence: `ac4f64e`; current v3 outcome extension: `5eabf7c`. TypeScript codec/runtime tests and Rust SessionHost gate tests cover valid Codex/Claude/Pi modes and rejection without sequence/cursor advancement. Native smoke is still required through CX-02, CL-02, and PI-02. Stop if provider event identity/provenance is ambiguous. |
