...
| **CX-01** | **Ready** | LC-02C, LC-02D, CAP-01B | Codex structured lifecycle adapter using supported app-server/public interfaces; coordinate identity work with DI-03. CX-01A adds an always-compiled, private fixture decoder in [`src-tauri/src/session_host/codex_app_server.rs`](src-tauri/src/session_host/codex_app_server.rs). | CX-01A and LC-02D fixture-verify the narrow public app-server v2 handshake, exact thread/turn/request identity, all terminal outcomes, and command/file approval request-resolution mapping through the shared gate. A sanitized Codex 0.144.5 smoke aligned the decoder with nested `turn.status`. Aggregate CX-01 remains incomplete: no process, prompt, readiness, discovery, or production protocol binding is included. | Evidence: `356b99b`, `5eabf7c`, `36385ee`. Native wire compatibility is observed, but no in-app provider workflow or **Provider verified** claim exists. Stop if only unsupported/private Codex state is available or identity cannot be bounded exactly. |
