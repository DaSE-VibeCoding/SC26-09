...
| **CX-01B** | **Ready** | LC-02D, CAP-01B; CX-01A evidence | Private Codex app-server process ownership and handshake foundation in [`src-tauri/src/session_host/codex_app_server.rs`](src-tauri/src/session_host/codex_app_server.rs), with only a minimal host-private atomic source/readiness helper if required. | An always-compiled but unregistered process-scoped supervisor directly owns a configured child, bounded JSONL and stderr draining, performs `initialize → initialized → thread/start` for one new thread, binds only matching `result.thread.id`, and produces normalized readiness/lifecycle envelopes through private gates. Fake-child fixtures prove correlation, atomicity, ordering, bounded failures, and graceful EOF/forced reap. | Rust fixtures plus a sanitized direct Codex 0.144.5 handshake/EOF smoke; transport compatibility only, never in-app **Provider verified** evidence. Stop on ambiguous identity, schema mismatch, readiness inference, unbounded data, orphan risk, raw-payload exposure, per-session process assumptions, or a need for production registration. |
| **CX-01** | **Roll-up** | CX-01B plus production binding prerequisites | Codex structured lifecycle adapter using supported app-server/public interfaces; coordinate identity work with DI-03. | Complete only when private decoding/supervision is followed by a production Codex binding, semantic prompt transport, terminal-less UI gating, version/schema fallback, and exact provider identity persistence. | Existing evidence: `356b99b`, `5eabf7c`, `36385ee`. Native wire compatibility is observed, but no in-app provider workflow or **Provider verified** claim exists. Stop if only unsupported/private Codex state is available or identity cannot be bounded exactly. |
