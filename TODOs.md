...
| **CAP-01** | **Ready** | LC-02C, CAP-01A | Provider capability and readiness contract in the agent registry and launch/resume flows. Carries the original agent-ready handshake backlog item. | CAP-01A's fixture foundation exists, but the aggregate remains incomplete until a real provider readiness producer/capability probe can set authoritative ready/auth/setup states without startup-output inference and unsupported capability gets explicit fallback copy from runtime evidence. | Native provider smoke is required before **Provider verified**. Stop if readiness is only inferable from TUI text, process liveness, terminal silence, or write success. |
| **CX-01** | **Ready** | LC-02C, LC-02D, CAP-01 | Codex structured lifecycle adapter using supported app-server/public interfaces; coordinate identity work with DI-03. CX-01A adds an always-compiled, private fixture decoder in [`src-tauri/src/session_host/codex_app_server.rs`](src-tauri/src/session_host/codex_app_server.rs). | CX-01A and LC-02D fixture-verify the narrow public app-server v2 handshake, exact thread/turn/request identity, all terminal outcomes, and command/file approval request-resolution mapping through the shared gate. A sanitized Codex 0.144.5 smoke aligned the decoder with nested `turn.status`. Aggregate CX-01 remains incomplete: no process, prompt, readiness, discovery, or production protocol binding is included. | Evidence: `356b99b`, `5eabf7c`, `36385ee`. Native wire compatibility is observed, but no in-app provider workflow or **Provider verified** claim exists. Stop if only unsupported/private Codex state is available or identity cannot be bounded exactly. |
